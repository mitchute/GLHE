...
from glhe.constants import SEC_IN_YEAR  # noqa
from glhe.constants import SEC_IN_HOUR  # noqa
from studies.aggregation.scripts.write_pbs import write_pbs  # noqa
from glhe.functions import load_json, write_json  # noqa
    ...
    for _, load in enumerate(loads):
        for idx_time, time in enumerate(run_times):
            ...
            for idx_ex, exp_rate in enumerate(exp_rates):
                for idx_sw, start_width in enumerate(start_widths):
                    for idx_ew, end_width in enumerate(end_widths):

                        depth, exp_rate, start_width, end_width = set_dynamic_parameters(time, exp_rate, start_width,
                                                                                         end_width)
